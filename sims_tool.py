...
players_file = st.sidebar.file_uploader("Players CSV", type=['csv'], key='players')
weekly_stats_file = st.sidebar.file_uploader("Weekly Stats CSV", type=['csv'], key='weekly_stats')
...
st.sidebar.markdown("**Optional: Team Stats**")
...
# Load data
if players_file and weekly_stats_file and matchup_file and lineups_file:
    try:
        # Load players.csv
        players_df = pd.read_csv(players_file)
        
        # Find player name column (could be 'Name' or 'Player')
        name_col = find_column(players_df, ['Player', 'Name', 'player', 'name'])
        if name_col is None:
            st.error("Players CSV missing player name column (Player, Name, etc.)")
            st.stop()
        ...
        proj_col = find_column(players_df, ['median_proj', 'projection', 'proj', 'fpts'])
        own_col = find_column(players_df, ['dk_ownership', 'ownership', 'proj_own', 'own%'])
        
        if not all(col in players_df.columns for col in required_player_cols):
            st.error(f"Players CSV missing required columns: {required_player_cols}")
            st.stop()
        
        if proj_col is None:
            st.error("Players CSV missing projection column (median_proj, projection, etc.)")
            st.stop()
        
        if own_col is None:
            st.error("Players CSV missing ownership column (dk_ownership, proj_own, etc.)")
            st.stop()
        
        # Standardize column names
        players_df = players_df.rename(columns={proj_col: 'median_proj', own_col: 'Proj_Own'})
        
        # Convert ownership to percentage if needed
        if players_df['Proj_Own'].max() <= 1.0:
            players_df['Proj_Own'] = players_df['Proj_Own'] * 100
        
        # Get ceiling and var_dk if available
        ceil_col = find_column(players_df, ['ceiling_proj', 'ceiling', 'ceil'])
        if ceil_col:
            players_df['ceiling_proj'] = players_df[ceil_col]
        else:
            players_df['ceiling_proj'] = players_df['median_proj'] * 1.5
        
        var_col = find_column(players_df, ['Var DK', 'var_dk', 'variance', 'var', 'std', 'stddev'])
        if var_col:
            players_df['var_dk'] = players_df[var_col]
        
        # Load weekly stats
        weekly_stats_df = pd.read_csv(weekly_stats_file)
        
        # Find player name column
        stats_name_col = find_column(weekly_stats_df, ['DK_List', 'Name', 'Player', 'name', 'player'])
        if stats_name_col is None:
            st.error("Weekly Stats CSV missing player name column (DK_List, Name, Player, etc.)")
            st.stop()
        
        # Find points column
        points_col = find_column(weekly_stats_df, ['DK_Points', 'DK Points', 'dk_points', 'points', 'fpts'])
        if points_col is None:
            st.error("Weekly Stats CSV missing points column (DK_Points, DK Points, etc.)")
            st.stop()
        
        # Standardize column names
        weekly_stats_df = weekly_stats_df.rename(columns={
            stats_name_col: 'Name',
            points_col: 'DK Points'
        })
        
        # Verify Week column exists
        if 'Week' not in weekly_stats_df.columns:
            st.error("Weekly Stats CSV missing 'Week' column")
            st.stop()
        ...
        if st.button("🚀 Run Simulations", type="primary"):
            with st.spinner(f"Running {n_sims:,} simulations..."):
                ...
                # Use 10% of field size for realistic field representation
                field_size_sim = max(150, field_size // 10)
                st.info(f"Generating {field_size_sim:,} field lineups (representing {field_size:,} full field)...")
                ...
                for sim_idx in range(n_sims):
                    ...
                    # Sample scores ONLY for players in lineups (much faster)
                    sim_scores = {}
                    ...
                    for _, player_row in players_in_lineups.iterrows():
                        ...
                        sim_scores[player_name] = score_after_env
                    
                    # Score all lineups (field + user)
                    all_scores = []
                    
                    # Field lineups
                    for field_lineup in field_lineups:
                        if use_correlations:
                            field_scores_adjusted = apply_correlations(field_lineup, sim_scores.copy(), players_df, env_state, qb_pass_corr, rng)
                            lineup_score = sum(field_scores_adjusted.get(p, 0) for p in field_lineup)
                        else:
                            lineup_score = sum(sim_scores.get(p, 0) for p in field_lineup)
                        
                        all_scores.append(lineup_score)
                    
                    # User lineups
                    user_scores = []
                    for user_lineup in user_lineups:
                        if use_correlations:
                            user_scores_adjusted = apply_correlations(user_lineup, sim_scores.copy(), players_df, env_state, qb_pass_corr, rng)
                            lineup_score = sum(user_scores_adjusted.get(p, 0) for p in user_lineup)
                        else:
                            lineup_score = sum(sim_scores.get(p, 0) for p in user_lineup)
                        
                        user_scores.append(lineup_score)
