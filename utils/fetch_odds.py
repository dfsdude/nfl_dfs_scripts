...
def main():
    ...
    project_root = os.path.dirname(script_dir)  # dfsdude-tools directory
    default_output = os.path.join(project_root, "data", "odds-api", "odds.csv")
    
    ap = argparse.ArgumentParser()
    ap.add_argument("--live-file", type=str, default=default_output)
    ...
    if parsed.empty:
        ...
        # Drop all-NA columns from both dataframes before concatenating to avoid FutureWarning
        if not prev.empty:
            prev = prev.dropna(axis=1, how='all')
