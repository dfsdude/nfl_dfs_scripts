...
def load_data():
    ...
    # Std dev calculation - CRITICAL for boom probabilities
    # Method 1: Use Var DK if available (most accurate)
    players["stddev_from_var"] = np.sqrt(players["var_dk"])
    
    # Method 2: Estimate from ceiling (assumes ceiling = 95th percentile = mean + 1.65*std)
    players["stddev_from_ceiling"] = ((players["ceiling_ows"] - players["proj"]) / 1.65).clip(lower=0)
    
    # Method 3: Position-based estimates for players with no data
    # These are empirical estimates from historical DFS variance by position
    def get_position_stddev(row):
        pos = row["position"]
        proj = row["proj"]
        
        # Position-specific variance multipliers (as % of projection)
        variance_mult = {
            "QB": 0.45,    # QBs are fairly consistent
            "RB": 0.60,    # RBs have high variance
            "WR": 0.65,    # WRs have highest variance
            "TE": 0.55,    # TEs moderate variance
            "DST": 0.70,   # DSTs are extremely volatile
            "D": 0.70
        }
        mult = variance_mult.get(pos, 0.55)
        return proj * mult
    
    players["stddev_position_est"] = players.apply(get_position_stddev, axis=1)
    
    # Use best available method in priority order:
    # 1. sqrt(var_dk) if var_dk exists and is valid
    # 2. ceiling-based if produces reasonable value (between 20-80% of projection)
    # 3. position-based estimate as fallback
    
    players["stddev"] = players["stddev_from_var"]
    
    # Fill NaN with ceiling-based estimates (if reasonable)
    mask_need_ceiling = players["stddev"].isna()
    ceiling_reasonable = (
        (players["stddev_from_ceiling"] >= players["proj"] * 0.2) & 
        (players["stddev_from_ceiling"] <= players["proj"] * 1.0)
    )
    players.loc[mask_need_ceiling & ceiling_reasonable, "stddev"] = \
        players.loc[mask_need_ceiling & ceiling_reasonable, "stddev_from_ceiling"]
    
    # Fill remaining NaN with position-based estimates
    players["stddev"] = players["stddev"].fillna(players["stddev_position_est"])
    
    # Ensure minimum stddev (at least 20% of projection for all players)
    players["stddev"] = players["stddev"].clip(lower=players["proj"] * 0.2)
    ...
    players["def_z_score"] = players["def_z_score"].fillna(0)  # Ensure no NaN values
    ...
    players["proj_adj"] = players["proj_adj"].fillna(players["proj"])
    players["ceiling_adj"] = players["ceiling_adj"].fillna(players["ceiling_ows"])
    players["stddev_adj"] = players["stddev_adj"].fillna(players["stddev"])
...
def get_stack(df_in, matchup_dict, stack_type, stack_positions, bringback_positions):
    ...
    for _, qb in qbs.iterrows():
        ...
        for combo in team_combos:
            ...
            for stack_all in stack_variations:
                ...
                total_ceiling_adj = sum(p.get("ceiling_adj", p.get("dk_ceiling", 0)) for p in stack_all)
                ...
                # Use proj_adj if available, fallback to proj
                stack_mean = sum(p.get("proj_adj", p.get("proj", p.get("dk", 0))) for p in stack_all)
                ...
                # Use stddev_adj if available, fallback to stddev, then estimate from proj
                stack_var = 0
                for p in stack_all:
                    if "stddev_adj" in p and not pd.isna(p["stddev_adj"]):
                        std = p["stddev_adj"]
                    elif "stddev" in p and not pd.isna(p["stddev"]):
                        std = p["stddev"]
                    else:
                        # Estimate stddev as 50% of projection if missing
                        proj = p.get("proj_adj", p.get("proj", p.get("dk", 10)))
                        std = proj * 0.5
                    stack_var += std ** 2
                ...
                if qb_idx is not None:
                    # Get QB stddev
                    if "stddev_adj" in stack_all[qb_idx] and not pd.isna(stack_all[qb_idx]["stddev_adj"]):
                        qb_std = stack_all[qb_idx]["stddev_adj"]
                    elif "stddev" in stack_all[qb_idx] and not pd.isna(stack_all[qb_idx]["stddev"]):
                        qb_std = stack_all[qb_idx]["stddev"]
                    else:
                        qb_proj = stack_all[qb_idx].get("proj_adj", stack_all[qb_idx].get("proj", stack_all[qb_idx].get("dk", 15)))
                        qb_std = qb_proj * 0.5
                    
                    for pc_idx in pass_catcher_indices:
                        # Get pass catcher stddev
                        if "stddev_adj" in stack_all[pc_idx] and not pd.isna(stack_all[pc_idx]["stddev_adj"]):
                            pc_std = stack_all[pc_idx]["stddev_adj"]
                        elif "stddev" in stack_all[pc_idx] and not pd.isna(stack_all[pc_idx]["stddev"]):
                            pc_std = stack_all[pc_idx]["stddev"]
                        else:
                            pc_proj = stack_all[pc_idx].get("proj_adj", stack_all[pc_idx].get("proj", stack_all[pc_idx].get("dk", 10)))
                            pc_std = pc_proj * 0.5
                ...
                for i in range(len(pass_catcher_indices)):
                    for j in range(i + 1, len(pass_catcher_indices)):
                        ...
                        if stack_all[idx_i]["team"] == stack_all[idx_j]["team"]:
                            # Get std for player i
                            if "stddev_adj" in stack_all[idx_i] and not pd.isna(stack_all[idx_i]["stddev_adj"]):
                                std_i = stack_all[idx_i]["stddev_adj"]
                            elif "stddev" in stack_all[idx_i] and not pd.isna(stack_all[idx_i]["stddev"]):
                                std_i = stack_all[idx_i]["stddev"]
                            else:
                                proj_i = stack_all[idx_i].get("proj_adj", stack_all[idx_i].get("proj", stack_all[idx_i].get("dk", 10)))
                                std_i = proj_i * 0.5
                            
                            # Get std for player j
                            if "stddev_adj" in stack_all[idx_j] and not pd.isna(stack_all[idx_j]["stddev_adj"]):
                                std_j = stack_all[idx_j]["stddev_adj"]
                            elif "stddev" in stack_all[idx_j] and not pd.isna(stack_all[idx_j]["stddev"]):
                                std_j = stack_all[idx_j]["stddev"]
                            else:
                                proj_j = stack_all[idx_j].get("proj_adj", stack_all[idx_j].get("proj", stack_all[idx_j].get("dk", 10)))
                                std_j = proj_j * 0.5
                ...
                # Boom target: Use a FIXED threshold based on historical GPP-winning stack scores
                # For 3-4 player stacks, winning lineups typically need 60-80+ combined points from core stack
                # We'll scale based on number of players in stack
                num_players = len(stack_all)
                
                # Base boom thresholds by stack size (empirical from GPP data)
                if num_players == 2:
                    stack_boom_target = 50  # QB+1: Need ~50 combined for elite outcome
                elif num_players == 3:
                    stack_boom_target = 65  # QB+2: Need ~65 combined
                elif num_players == 4:
                    stack_boom_target = 80  # QB+3: Need ~80 combined
                else:
                    stack_boom_target = 95  # QB+3+BB: Need ~95 combined
                
                # Calculate boom probability based on FIXED target
                # Now high-variance stacks will have higher boom% than low-variance stacks
                if stack_std > 0:
                    stack_boom_prob = 1 - normal_cdf(stack_boom_target, stack_mean, stack_std)
                    stack_boom_prob = min(max(stack_boom_prob, 0), 1)
                else:
                    # If stddev is 0, deterministic outcome
                    stack_boom_prob = 1.0 if stack_mean >= stack_boom_target else 0.0
    ...
    # For Thanksgiving/short slates, show more stacks per team since there are fewer teams
    # Detect slate size based on unique teams
    if not df_results.empty:
        num_teams = df_results['Team'].nunique()
        if num_teams <= 6:
            # Thanksgiving slate (3 games = 6 teams) - show top 10 per team
            top_n = 10
        elif num_teams <= 10:
            # Small slate - show top 6 per team
            top_n = 6
        else:
            # Full slate - show top 4 per team
            top_n = 4
...
if view_mode == "🧱 Top Stacks":
    ...
    if df_filtered.empty:
        ...
        if stack_df.empty:
            ...
            # Debug info for first stack
            if len(stack_df) > 0:
                with st.expander("🔍 Debug: Top Stack Boom Calculation", expanded=False):
                    top_stack = stack_df.iloc[0]
                    st.write(f"**Players:** {top_stack['Players']}")
                    st.write(f"**Total Projection (stack_mean):** {top_stack['Total_Proj']:.2f}")
                    st.write(f"**Boom Target (fixed by stack size):** {top_stack['Boom_Target']:.1f}")
                    
                    # Calculate implied z-score
                    implied_std = (top_stack['Boom_Target'] - top_stack['Total_Proj'])
                    st.write(f"**Gap to Boom:** {implied_std:.1f} points")
                    
                    st.write(f"**Stack Boom% (P(score > {top_stack['Boom_Target']:.0f})):** {top_stack['Stack_Boom%']:.1f}%")
                    st.write(f"**Total Ownership:** {top_stack['Total_Own%']:.1f}%")
                    st.write(f"**Stack Leverage (Boom% - Own%):** {top_stack['Stack_Leverage']:.1f}")
                    
                    st.divider()
                    
                    # Show individual player details
                    player_names = top_stack['Players'].split(", ")
                    st.write("**Individual Player Variance Components:**")
                    for pname in player_names[:4]:  # Show first 4 players
                        player_data = df_filtered[df_filtered['name'] == pname]
                        if not player_data.empty:
                            p = player_data.iloc[0]
                            st.write(f"- **{pname}**: Proj={p['proj']:.1f}, Ceiling={p['ceiling_ows']:.1f}, StdDev={p['stddev']:.2f}")
                    
                    st.divider()
                    st.caption("📊 **Boom% varies by:**")
                    st.caption("- Stack variance (higher variance = higher boom potential)")
                    st.caption("- Player correlations (QB-WR stacks have higher variance)")
                    st.caption("- Individual player volatility (ceiling - projection)")
                    
                    # Show variance of different stacks
                    if len(stack_df) >= 3:
                        st.write("\n**Boom% Distribution Across Stacks:**")
                        st.write(f"Min: {stack_df['Stack_Boom%'].min():.1f}% | Max: {stack_df['Stack_Boom%'].max():.1f}% | Median: {stack_df['Stack_Boom%'].median():.1f}%")
